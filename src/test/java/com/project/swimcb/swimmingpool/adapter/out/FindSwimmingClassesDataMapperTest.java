...
class FindSwimmingClassesDataMapperTest {
  ...
  @Test
  @DisplayName("요일 목록이 주어지면 올바른 비트 벡터로 변환한다.")
  ...
  @Test
  @DisplayName("거리순 정렬 조건이 주어지면 거리 기준 정렬자를 반환한다.")
  ...
  @Test
  @DisplayName("가격순 정렬 조건이 주어지면 가격 기준 정렬자를 반환한다.")
  void shouldReturnPriceOrderSpecifierWhenCreatePriceOrderSpecifier() {
    ...
    // 기격 기준 반환값은 정적 참조라 직접 비교하기 어려우므로 null이 아닌지만 확인
  }

  @Nested
  @DisplayName("강습형태와 강습구분 조건 테스트")
  class ClassTypeAndSubTypeTest {
    ...
    @Test
    @DisplayName("강습형태가 없으면 false를 반환하는 필터 조건을 생성한다.")
