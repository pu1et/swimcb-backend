...
class SetFreeSwimmingReservationBlockedInteractorTest {

  @Mock
  private SetFreeSwimmingReservationBlockedDsGateway gateway;

  @InjectMocks
  private SetFreeSwimmingReservationBlockedInteractor interactor;

  @Nested
  @DisplayName("자유수영 예약 마감 설정 시")
  class DescribeSetFreeSwimmingReservationBlocked {
    ...
    class ContextWithExistingFreeSwimmingDayStatuses {
      ...
      @Test
      @DisplayName("마감 설정에 성공한다")
      void shouldSetFreeSwimmingReservationBlockedWhenAllDayStatusesExist() {
        // given
        val command = SetFreeSwimmingReservationBlockedCommand.builder()
            .swimmingPoolId(1L)
        ...
        willDoNothing().given(gateway).setFreeSwimmingReservationBlocked(command);

        // when
        interactor.setFreeSwimmingReservationBlocked(command);
        ...
        then(gateway).should().setFreeSwimmingReservationBlocked(command);
      }
    ...
    class ContextWithNonExistentFreeSwimmingDayStatuses {
      ...
      void shouldThrowExceptionWhenFreeSwimmingDayStatusNotFound() {
        ...
        // when
        // then
        assertThatThrownBy(() -> interactor.setFreeSwimmingReservationBlocked(command))
